...
function parseArgs(): { gameNum: number | null } {
...
async function stripCachedElosFromIndex(): Promise<void> {
  ...
  index.chernov = index.chernov.map((entry) => {
...
async function main(): Promise<void> {
  const { gameNum } = parseArgs();
  const indexRaw = await fs.readFile(INDEX_PATH, "utf8");
  const index = JSON.parse(indexRaw) as LessonIndex;
  const targets = index.chernov.filter((entry) => gameNum === null || entry.gameNum === gameNum);
